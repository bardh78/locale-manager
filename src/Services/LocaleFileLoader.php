...
namespace Autoluminescent\LocaleManager\Services;
...
class LocaleFileLoader implements Loader
{
    /**
        ...
        return collect(array_merge($this->jsonPaths, $this->paths))
            ->reduce(function ($output, $path) use ($locale) {
                if ($this->files->exists($full = "{$path}/{$locale}.json")) {
                    ...
                    // Flatten the array to support dot notation
                    $flattened = $this->arrayDot($decoded);
